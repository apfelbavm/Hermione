...
function isSelectionConnected(
  ...
  for (const c of graph.connections) {
    if (
      selectedNodeIds.has(c.fromNode) &&
      selectedNodeIds.has(c.toNode) &&
      c.fromNode !== c.toNode
    ) {
...
export function canCollapseSelectionToFunction(
  rootGraph: Graph,
  graph: Graph,
  selectedNodeIds: ReadonlySet<string>,
  variables: Variable[],
  ...
  const { incomingExec, outgoingExec, outgoingData } = classifyBoundary(
    graph,
    selectedNodeIds,
    variables,
    functions,
...
export function collapseSelectionToFunction(
  ...
  const { incomingExec, outgoingExec, incomingData, outgoingData } =
    classifyBoundary(graph, selectedNodeIds, variables, functions, scripts);
  ...
  const consumedConnectionIds = new Set(
    [...internalConnections, ...incomingExec, ...outgoingExec, ...incomingData, ...outgoingData].map(
      (c) => c.id,
    ),
  );
  ...
  function groupByExternalEndpoint(
    conns: Connection[],
  ...
  for (const group of groupByExternalEndpoint(incomingData, (c) => `${c.fromNode}:${c.fromPin}`)) {
    ...
    const targetPinDef = targetNode
      .resolvePinDefs(bodyVariables, rootGraph.functions, scripts)
      .find((p) => p.id === first.toPin)!;
    ...
    for (const c of group) {
      connectPins(fn.body, bodyVariables, rootGraph.functions, {
  ...
  for (const group of groupByExternalEndpoint(outgoingData, (c) => `${c.fromNode}:${c.fromPin}`)) {
    ...
    const sourcePinDef = sourceNode
      .resolvePinDefs(bodyVariables, rootGraph.functions, scripts)
      .find((p) => p.id === first.fromPin)!;
  ...
  const callNode = NodeInstance.createNodeInstance(
    "function.call",
    { x: minX, y: minY },
    callPinDefs,
    undefined,
  ...
  for (const c of incomingExec) {
    connectPins(graph, variables, rootGraph.functions, {
      fromNode: c.fromNode,
      fromPin: c.fromPin,
      toNode: callNode.id,
      toPin: "exec-in",
