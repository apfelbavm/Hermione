...
export async function executeDeployedFlow(projectId: string, flowId: string, log: (message: string) => void): Promise<ExecuteFlowResult> {
  ...
  try {
    ...
    if (!runTrigger) {
      return { success: false, error: 'The "EventRun" function does not exist in this graph.', outputs: {} };
    }
    ...
    const instance = new CompiledFlow(log);
    const fn = (instance[runTrigger.functionName] as () => Promise<Record<string, unknown> | void>).bind(instance);

    const returned = await fn();
    return { success: true, error: "", outputs: returned && typeof returned === "object" ? returned : {} };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err), outputs: {} };
  }
}
