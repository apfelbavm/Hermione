...
import { changeNodeElementType, connectPins, createNodeInstance } from "../engine/graphMutations";
...
describe("array.make", () => {
  it("exposes a configurable element type and starts with one removable entry", () => {
    ...
    const node = createNodeInstance("array.make", { x: 0, y: 0 }, def.pins, "make");
  ...
  it("adds a new entry via addInstancePinEntry and assembles the array from all entries", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { "entry-0": 1, "entry-1": 2 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
    expect(outputs.result).toEqual([1, 2]);

    const compiled = def.compileEvaluate!({ node, inputs: { "entry-0": "1", "entry-1": "2" }, graph: createEmptyGraph("g", "r") });
...
describe("array.length", () => {
  it("returns 0 for an unwired (empty array) input and the count otherwise", async () => {
    ...
    const node = createNodeInstance("array.length", { x: 0, y: 0 }, def.pins, "len");
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3] }, ctx: ctxFor(createEmptyGraph("g", "r")) });
    expect(outputs.length).toBe(3);
    const compiled = def.compileEvaluate!({ node, inputs: { array: JSON.stringify([1, 2, 3]) }, graph: createEmptyGraph("g", "r") });
...
describe("array.get", () => {
  ...
  const node = createNodeInstance("array.get", { x: 0, y: 0 }, def.pins, "get");

  it("returns the element and found=true for an in-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: ["a", "b", "c"], index: 1 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
  ...
  it("returns the type's default and found=false for an out-of-range index, without throwing", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: ["a", "b"], index: 5 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
  ...
  it("compiles to the same element/found values", () => {
    const compiled = def.compileEvaluate!(
      { node, inputs: { array: JSON.stringify(["a", "b", "c"]), index: "1" }, graph: createEmptyGraph("g", "r") },
    );
...
describe("array.set", () => {
  ...
  const node = createNodeInstance("array.set", { x: 0, y: 0 }, def.pins, "set");

  it("replaces the element at an in-range index without mutating the input array", async () => {
    const original = [1, 2, 3];
    const outputs = await def.evaluate!({ node, inputs: { array: original, index: 1, value: 99 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
  ...
  it("reports success=false and returns the array unchanged for an out-of-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 9, value: 99 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.add", () => {
  it("appends the item and reports its new index", async () => {
    ...
    const node = createNodeInstance("array.add", { x: 0, y: 0 }, def.pins, "add");
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], item: 3 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.append", () => {
  it("concatenates two arrays", async () => {
    ...
    const node = createNodeInstance("array.append", { x: 0, y: 0 }, def.pins, "append");
    const outputs = await def.evaluate!({ node, inputs: { a: [1, 2], b: [3, 4] }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.insert", () => {
  it("inserts at a valid index", async () => {
    ...
    const node = createNodeInstance("array.insert", { x: 0, y: 0 }, def.pins, "insert");
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3], index: 1, item: 99 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
  ...
  it("clamps an out-of-range index instead of throwing", async () => {
    const def = getNodeDef("array.insert");
    const node = createNodeInstance("array.insert", { x: 0, y: 0 }, def.pins, "insert");
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 99, item: 3 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.removeAt", () => {
  ...
  const node = createNodeInstance("array.removeAt", { x: 0, y: 0 }, def.pins, "removeAt");

  it("removes the element at an in-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3], index: 1 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 3], success: true });
  });

  it("reports success=false for an out-of-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 9 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 2], success: false });
  });
});
...
describe("array.removeItem", () => {
  ...
  const node = createNodeInstance("array.removeItem", { x: 0, y: 0 }, def.pins, "removeItem");

  it("removes the first matching item by value", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3, 2], item: 2 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 3, 2], removed: true });
  });

  it("reports removed=false when the item isn't present", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], item: 99 }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.clear / contains / findIndex / isEmpty / reverse", () => {
  it("Clear always returns an empty array", async () => {
    ...
    const node = createNodeInstance("array.clear", { x: 0, y: 0 }, def.pins, "clear");
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3] }, ctx: ctxFor(createEmptyGraph("g", "r")) })).result).toEqual([]);
  });

  it("Contains finds a deep-equal item", async () => {
    const def = getNodeDef("array.contains");
    const node = createNodeInstance("array.contains", { x: 0, y: 0 }, def.pins, "contains");
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 2 }, ctx: ctxFor(createEmptyGraph("g", "r")) })).contains).toBe(true);
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 9 }, ctx: ctxFor(createEmptyGraph("g", "r")) })).contains).toBe(false);
  });

  it("Find Index returns -1 when absent", async () => {
    const def = getNodeDef("array.findIndex");
    const node = createNodeInstance("array.findIndex", { x: 0, y: 0 }, def.pins, "findIndex");
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 9 }, ctx: ctxFor(createEmptyGraph("g", "r")) })).index).toBe(-1);
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 2 }, ctx: ctxFor(createEmptyGraph("g", "r")) })).index).toBe(1);
  });

  it("Is Empty reflects the array's length", async () => {
    const def = getNodeDef("array.isEmpty");
    const node = createNodeInstance("array.isEmpty", { x: 0, y: 0 }, def.pins, "isEmpty");
    expect((await def.evaluate!({ node, inputs: { array: [] }, ctx: ctxFor(createEmptyGraph("g", "r")) })).isEmpty).toBe(true);
    expect((await def.evaluate!({ node, inputs: { array: [1] }, ctx: ctxFor(createEmptyGraph("g", "r")) })).isEmpty).toBe(false);
  });

  it("Reverse returns a reversed copy without mutating the input", async () => {
    const def = getNodeDef("array.reverse");
    const node = createNodeInstance("array.reverse", { x: 0, y: 0 }, def.pins, "reverse");
    const original = [1, 2, 3];
    const outputs = await def.evaluate!({ node, inputs: { array: original }, ctx: ctxFor(createEmptyGraph("g", "r")) });
...
describe("array.forEach", () => {
  it("runs the loop-body chain once per element, exposing Element and Index, then fires Completed", async () => {
    ...
    graph.nodes.push(createNodeInstance("debug.print", { x: 0, y: 0 }, printBody.pins, "printElement"));
    const printDone = createNodeInstance("debug.print", { x: 0, y: 0 }, printBody.pins, "printDone");
...
describe("changeNodeElementType", () => {
  it("switches an array node's element type, disconnects wires touching it, and resets its pins to the new type's default", () => {
    ...
    const makeNode = createNodeInstance("array.make", { x: 0, y: 0 }, makeDef.pins, "make");
    graph.nodes.push(makeNode);
    const lengthDef = getNodeDef("array.length");
    const lengthNode = createNodeInstance("array.length", { x: 100, y: 0 }, lengthDef.pins, "len");
    graph.nodes.push(lengthNode);

    // Both are Array<number> right now, so this wire is valid.
    connectPins(graph, [], [], { fromNode: "make", fromPin: "result", toNode: "len", toPin: "array" });
    expect(graph.connections).toHaveLength(1);
    expect(lengthNode.pins.array.connectionId).toBeDefined();

    changeNodeElementType(graph, [], [], "len", { elementType: "string" });
    ...
    expect(pins.find((p) => p.id === "array")).toMatchObject({ type: "string", container: "array" });
  });
  ...
  it("preserves a Make Array node's entry count across an element-type change, resetting only their values", () => {
    ...
    changeNodeElementType(graph, [], [], "make", { elementType: "string" });
