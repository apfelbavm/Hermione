...
describe("array.make", () => {
  ...
  it("adds a new entry via addInstancePinEntry and assembles the array from all entries", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { "entry-0": 1, "entry-1": 2 }, ctx: ctxFor(new Graph("g", "r")) });
    expect(outputs.result).toEqual([1, 2]);

    const compiled = def.compileEvaluate!({ node, inputs: { "entry-0": "1", "entry-1": "2" }, graph: new Graph("g", "r") });
...
describe("array.length", () => {
  it("returns 0 for an unwired (empty array) input and the count otherwise", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3] }, ctx: ctxFor(new Graph("g", "r")) });
    expect(outputs.length).toBe(3);
    const compiled = def.compileEvaluate!({ node, inputs: { array: JSON.stringify([1, 2, 3]) }, graph: new Graph("g", "r") });
...
describe("array.get", () => {
  ...
  it("returns the element and found=true for an in-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: ["a", "b", "c"], index: 1 }, ctx: ctxFor(new Graph("g", "r")) });
  ...
  it("returns the type's default and found=false for an out-of-range index, without throwing", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: ["a", "b"], index: 5 }, ctx: ctxFor(new Graph("g", "r")) });
  ...
  it("compiles to the same element/found values", () => {
    const compiled = def.compileEvaluate!(
      { node, inputs: { array: JSON.stringify(["a", "b", "c"]), index: "1" }, graph: new Graph("g", "r") },
    );
...
describe("array.set", () => {
  ...
  it("replaces the element at an in-range index without mutating the input array", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { array: original, index: 1, value: 99 }, ctx: ctxFor(new Graph("g", "r")) });
  ...
  it("reports success=false and returns the array unchanged for an out-of-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 9, value: 99 }, ctx: ctxFor(new Graph("g", "r")) });
...
describe("array.add", () => {
  it("appends the item and reports its new index", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], item: 3 }, ctx: ctxFor(new Graph("g", "r")) });
...
describe("array.append", () => {
  it("concatenates two arrays", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { a: [1, 2], b: [3, 4] }, ctx: ctxFor(new Graph("g", "r")) });
...
describe("array.insert", () => {
  it("inserts at a valid index", async () => {
    ...
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3], index: 1, item: 99 }, ctx: ctxFor(new Graph("g", "r")) });
  ...
  it("clamps an out-of-range index instead of throwing", async () => {
    const def = getNodeDef("array.insert");
    const node = createNodeInstance("array.insert", { x: 0, y: 0 }, def.pins, "insert");
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 99, item: 3 }, ctx: ctxFor(new Graph("g", "r")) });
...
describe("array.removeAt", () => {
  ...
  it("removes the element at an in-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3], index: 1 }, ctx: ctxFor(new Graph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 3], success: true });
  });

  it("reports success=false for an out-of-range index", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], index: 9 }, ctx: ctxFor(new Graph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 2], success: false });
  });
});
...
describe("array.removeItem", () => {
  ...
  it("removes the first matching item by value", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2, 3, 2], item: 2 }, ctx: ctxFor(new Graph("g", "r")) });
    expect(outputs).toEqual({ result: [1, 3, 2], removed: true });
  });

  it("reports removed=false when the item isn't present", async () => {
    const outputs = await def.evaluate!({ node, inputs: { array: [1, 2], item: 99 }, ctx: ctxFor(new Graph("g", "r")) });
...
describe("array.clear / contains / findIndex / isEmpty / reverse", () => {
  it("Clear always returns an empty array", async () => {
    ...
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3] }, ctx: ctxFor(new Graph("g", "r")) })).result).toEqual([]);
  });
  ...
  it("Contains finds a deep-equal item", async () => {
    ...
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 2 }, ctx: ctxFor(new Graph("g", "r")) })).contains).toBe(true);
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 9 }, ctx: ctxFor(new Graph("g", "r")) })).contains).toBe(false);
  });
  ...
  it("Find Index returns -1 when absent", async () => {
    ...
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 9 }, ctx: ctxFor(new Graph("g", "r")) })).index).toBe(-1);
    expect((await def.evaluate!({ node, inputs: { array: [1, 2, 3], item: 2 }, ctx: ctxFor(new Graph("g", "r")) })).index).toBe(1);
  });
  ...
  it("Is Empty reflects the array's length", async () => {
    ...
    expect((await def.evaluate!({ node, inputs: { array: [] }, ctx: ctxFor(new Graph("g", "r")) })).isEmpty).toBe(true);
    expect((await def.evaluate!({ node, inputs: { array: [1] }, ctx: ctxFor(new Graph("g", "r")) })).isEmpty).toBe(false);
  });
  ...
  it("Reverse returns a reversed copy without mutating the input", async () => {
    ...
    const original = [1, 2, 3];
    const outputs = await def.evaluate!({ node, inputs: { array: original }, ctx: ctxFor(new Graph("g", "r")) });
