...
registerNode({
  ...
  compileEvaluate: ({ inputs }) => ({ result: `String(Number(${inputs.value}))` }),
});
...
registerNode({
  ...
  evaluate: ({ inputs }) => ({ result: String(Boolean(inputs.value ?? false)) }),
  compileEvaluate: ({ inputs }) => ({ result: `String(Boolean(${inputs.value}))` }),
});
...
registerNode({
  ...
  compileEvaluate: ({ inputs }) => ({ result: `JSON.stringify(${inputs.value})` }),
});
...
const APPEND_RESULT_PIN: PinDef = { id: "result", label: "Result", type: "string", direction: "output" };
...
registerNode({
  ...
  group: "HermioneString",
  pins: [
    { id: `${ENTRY_PREFIX}0`, label: "String 1", type: "string", direction: "input", defaultValue: "" },
    { id: `${ENTRY_PREFIX}1`, label: "String 2", type: "string", direction: "input", defaultValue: "" },
    APPEND_RESULT_PIN,
  ],
