...
13. Two purely-visual, non-execution-affecting ways to document a graph, use either when it genuinely helps a human reading it (not on every request): "description" on create_node/update_node sets ONE node instance's own note (e.g. explaining why this particular node is configured this way); create_comment_box (in graph.apply_changes) draws a labeled box around a whole section (e.g. "Send the notification email"). Neither is a substitute for real nodes or connections.
