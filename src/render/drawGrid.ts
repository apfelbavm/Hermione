...
export function drawGrid(
  ...
  if (spacing >= 4) {
    ...
    for (
      let wx = startX, sx = (wx - camera.x) * camera.zoom;
      sx < width;
      wx += MINOR_SPACING, sx = (wx - camera.x) * camera.zoom
    ) {
      ctx.strokeStyle =
        col % MAJOR_EVERY === 0
          ? Colors.GRID_LINE_MAJOR
          : Colors.GRID_LINE_MINOR;
      ctx.beginPath();
      ctx.moveTo(sx, 0);
    ...
    for (
      let wy = startY, sy = (wy - camera.y) * camera.zoom;
      sy < height;
      wy += MINOR_SPACING, sy = (wy - camera.y) * camera.zoom
    ) {
      ctx.strokeStyle =
        row % MAJOR_EVERY === 0
          ? Colors.GRID_LINE_MAJOR
