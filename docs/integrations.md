...
Current providers: Dropbox, GitHub, Google (Admin/Calendar/Docs/Drive/Gmail/Sheets/Auth), Jira, Facebook, Azure Storage, Microsoft 365. Plus generic protocol nodes not tied to one vendor: `http.ts`, `soap.ts`, `sftp.ts`, `odata.ts`, `xml.ts`, `csv.ts`.
