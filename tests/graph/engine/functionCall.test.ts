...
import { createExecutionContext, runExecFrom, runFunctionCall } from "@hermione/graph/engine/executor";
import { addFunctionInput, addFunctionOutput, addVariable, connectPins, createFunctionDef, nextId } from "@hermione/graph/engine/graphMutations";
...
describe("function calls", () => {
  ...
  it("caller continues even if the function body never reaches a Return node, using declared defaults", async () => {
    ...
    const logs: string[] = [];
    const ctx = createExecutionContext(rootGraph, {
  ...
  it("a function's local variable is isolated from a global variable and from other calls", async () => {
    const rootGraph = new Graph("root", "test");
    const globalVar: Variable = {
      id: "shared-id-does-not-collide",
      name: "Counter",
      type: "string",
      defaultValue: "global",
    };
  ...
  it("creates a Return node by default, and Entry/Return can never be removed", () => {
    ...
    fn.body.removeNode([], [], entryNode!.id);
    fn.body.removeNode([], [], returnNode!.id);
